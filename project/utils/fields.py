...
class FieldDataset(Dataset):
    ...
    def __init__(self, in_patterns, tgt_patterns,
        ...
        if rank == 0:
            print('len of in:', len(self.in_files), flush=True)
            print('len of tgt:', len(self.tgt_files), flush=True)

            #assume two fields in input
            for i, (in_files, tgt_files) in enumerate(zip(self.in_files, self.tgt_files)):

                if i < 5:
                    print(f'input files {i}:', in_files, flush=True)
                    print(f'target files {i}:', tgt_files, flush=True)
                
                dis_lr, vel_lr = in_files
                dis_hr, vel_hr = tgt_files

                print('removing common prefix and suffix:', flush=True)

                #remove .npy, take last 12 characters, split by _, convert to list of int
                def process_filename(f):
                    f = os.path.basename(f)
                    f = f[:-4]
                    f = f[-12:]
                    f = f.split('_')
                    f = [int(x) if x.lstrip('-').isdigit() else x for x in f]
                    return f
                
                dis_lr = process_filename(dis_lr)
                vel_lr = process_filename(vel_lr)
                dis_hr = process_filename(dis_hr)
                vel_hr = process_filename(vel_hr)

                if i < 5:
                    print('dis_lr:', dis_lr, flush=True)
                    print('vel_lr:', vel_lr, flush=True)
                    print('dis_hr:', dis_hr, flush=True)
                    print('vel_hr:', vel_hr, flush=True)

                print('tests start')

                if not len(dis_lr) == 3: print('error 1')
                if not len(dis_lr) == len(vel_lr) == len(dis_hr) == len(vel_hr): print('error 2')

                if not dis_lr == vel_lr: print('error 3')
                if not dis_hr == vel_hr: print('error 4')

                #multiply all the values by 2
                dis_lr = [x*2 for x in dis_lr]
                vel_lr = [x*2 for x in vel_lr]
                
                if i < 5:
                    print('dis_lr x2:', dis_lr, flush=True)
                    print('vel_lr x2:', vel_lr, flush=True)

                if not dis_lr == dis_hr: print('error 5')
                if not vel_lr == vel_hr: print('error 6')
    ...
    def assemble(self, label, chan, patches, paths):
        ...
        if self.scale_factor != 1:
            print(f'scale factor {self.scale_factor} > 1 not supported in assemble')
...
def crop(fields, anchor, crop, pad):
    ...
    for d, (a, c, (p0, p1), s) in enumerate(zip(anchor, crop, pad, size)):
        ...
        #check if there are values outside the original field #NOTE: uncommented during training
        #if i.min() < 0 or i.max() >= s:
        #    raise RuntimeError('crop + pad goes beyond field size')

        i %= s #NOTE: commented this during training
        i = i.reshape((-1,) + (1,) * (ndim - d - 1))
        ind.append(i)
    ind = tuple(ind)
