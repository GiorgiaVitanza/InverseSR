...
path_fisico = r"C:/Modelli 3D/InverseSR - Astro/data/trained_models_astro/vae/data/model.pth"
...
try:
    ...
    # --- Ricerca z_channels ---
    # Spesso i modelli VAE hanno un layer chiamato 'quant_conv'
    # che definisce la dimensione del collo di bottiglia (bottleneck).
    
    latent_keys = [k for k in state_dict.keys() if "quant_conv.weight" in k]
    
    z_channels = "N/D"
    if latent_keys:
        # Prendiamo la prima occorrenza trovata
        shape = state_dict[latent_keys[0]].shape
        # In un layer conv [Out, In, D, H, W], Out rappresenta la profondità latente
        # Nota: Se è un VAE standard, Out potrebbe essere 2*z_channels (per mean e logvar)
        z_channels = shape[0] 
        print(f"Sospetto layer latente trovato: '{latent_keys[0]}'")
        print(f"Z-Channels stimati (latenti): {z_channels}")
    ...
    # --- Salvataggio Report ---
    file_path = "parametri_modello_vae_3d.txt"
    with open(file_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Z-Channels (Latenti stimati): {z_channels}\n")
        f.write("="*50 + "\n\n")
